...
public class SumOfCompletionTime{
	...
	public Job[] readFile() throws IOException{
		...
		while((s = in.readLine()) != null){
			...
			jobs[i].difference = jobs[i].weight - jobs[i].length;
			i ++;
		}
		in.close();
		return jobs;
	...
	public Job[] merge(Job[] left, Job[] right){
		...
		for( ; i < res.length  && j < left.length && k < right.length; i ++){
			...
			if(left[j].difference > right[k].difference 
					||(left[j].difference == right[k].difference && left[j].weight > right[k].weight) ){
				res[i] = left[j ++];
			}
			else
				res[i] = right[k ++];
	...
	public static void main(String[] args) throws IOException{
		...
		SumOfCompletionTime so = new SumOfCompletionTime();
		//read in data
		Job[] jobs = so.readFile();
		//output the array
		for(int i = 0 ; i < jobs.length ; i ++){
			System.out.print(jobs[i].weight + "\t" + jobs[i].length + "\t" + jobs[i].difference + "\n");
		}
		System.out.println("*******************");
		//sort the jobs
